...
/* GET home page. */
router.post('/bankoffer_sumbit',upload.any(), function(req, res, next) {
    var f=[]
    req.files.map((item)=>{
        f.push(item.filename)
    })
  try{
    pool.query("insert into bankoffers( status, filenames)values(?,?)",[req.body.status,f+""],function(error,result){
        if(error){
            console.log(error)
